...
| --- | --- | --- | --- |
| `fact_sales` | Invoice-line | `invoice_line_id` | Measures each item purchased on an invoice. Stores quantity, unit price, and revenue (`line_total`). |
...
* **Surrogate line key**: `invoice_line_id` is generated by concatenating invoice number with the row position, ensuring uniqueness even when the source lacks a line identifier.
* **Customer dimension**: The dataset provides limited customer attributes. We retain `country` and normalize `CustomerID` into `dim_customer` for future enrichment.
...
* **Date dimension**: Derives year/month/day/week fields and a numeric `date_key` (`YYYYMMDD`) to simplify joins and enable standard calendar analytics.

## SQL DDL

See `sql/create_tables.sql` for table creation statements.
